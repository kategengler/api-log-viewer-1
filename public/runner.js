...
// Initialize more QUnit.config and QUnit.urlParams
  (function() {
    var i, current,
        location = top.window.location || { search: "", protocol: "file:" },
...
var origQUnitUrl = QUnit.url;
QUnit.url = function( params ) {
  var origUrl = origQUnitUrl(params);
  var origLoc = window.location.protocol + "//" + window.location.host +
                window.location.pathname;
  var newLoc = top.window.location.protocol + "//" + top.window.location.host +
               top.window.location.pathname;
  return origUrl.replace(origLoc, newLoc);
};


(function() {

  var runnerWindow = window;
  var reporterWindow = top.window;
  var reporterDocument = reporterWindow.document;
// Deprecated QUnit.init - Ref #530
// Re-initialize the configuration options
  QUnit.init = function() {
    var tests, banner, result, qunit,
    ...
    if ( qunit ) {
      qunit.innerHTML =
      "<h1 id='qunit-header'>" + escapeText( reporterDocument.title ) + "</h1>" +
      "<h2 id='qunit-banner'></h2>" +
      "<div id='qunit-testrunner-toolbar'></div>" +
      "<h2 id='qunit-userAgent'></h2>" +
      "<ol id='qunit-tests'></ol>";
    ...
    if ( tests ) {
      result = reporterDocument.createElement( "p" );
      result.id = "qunit-testresult";
      result.className = "result";
      tests.parentNode.insertBefore( result, tests );
      result.innerHTML = "Running...<br/>&nbsp;";
...
// Don't load the HTML Reporter on non-Browser environments
  ...
  var config = QUnit.config,
      hasOwn = Object.prototype.hasOwnProperty,
      defined = {
        document: typeof reporterWindow.document !== "undefined",
        sessionStorage: (function() {
          var x = "qunit-test-string";
          try {
            sessionStorage.setItem( x, x );
  ...
  function id( name ) {
    return defined.document && reporterDocument.getElementById && reporterDocument.getElementById( name );
  }
  ...
  function toolbarUrlConfigContainer() {
    var urlConfigContainer = reporterDocument.createElement( "span" );
    ...
    addEvents( urlConfigContainer.getElementsByTagName( "input" ), "click", function( event ) {
      ...
      params[ target.name ] = target.checked ?
        target.defaultValue || true :
        undefined;
      reporterWindow.location = QUnit.url( params );
    });
    addEvents( urlConfigContainer.getElementsByTagName( "select" ), "change", function( event ) {
      var params = {},
          target = event.target || event.srcElement;
  ...
  function toolbarModuleFilter() {
    var moduleFilter = reporterDocument.createElement( "span" ),
        moduleFilterHtml = toolbarModuleFilterHtml();
    ...
    addEvent( moduleFilter.lastChild, "change", function() {
      ...
      reporterWindow.location = QUnit.url({
        module: ( selectedModule === "" ) ? undefined : selectedModule,
  ...
  function toolbarFilter() {
    var testList = id( "qunit-tests" ),
        filter = reporterDocument.createElement( "input" );
  ...
  function toolbarLabel() {
    var label = reporterDocument.createElement( "label" );
    label.setAttribute( "for", "qunit-filter-pass" );
    label.setAttribute( "title", "Only show tests and assertions that fail. Stored in sessionStorage." );
    label.innerHTML = "Hide passed tests";
...
// HTML Reporter initialization and load
  ...
  QUnit.done(function( details ) {
    ...
    if ( config.altertitle && defined.document && reporterDocument.title ) {

      // show ✖ for good, ✔ for bad suite result in title
      // use escape sequences in case file gets loaded with non-utf-8-charset
      reporterDocument.title = [
        ( details.failed ? "\u2716" : "\u2714" ),
        reporterDocument.title.replace( /^[\u2714\u2716] /i, "" )
      ].join( " " );
    }
    ...
    // scroll back to top to show results
    if ( config.scrolltop && reporterWindow.scrollTo ) {
      reporterWindow.scrollTo( 0, 0 );
    }
  });
  ...
  QUnit.testStart(function( details ) {
    ...
    if ( tests ) {
      b = reporterDocument.createElement( "strong" );
      b.innerHTML = name;

      a = reporterDocument.createElement( "a" );
      a.innerHTML = "Rerun";
      a.href = QUnit.url({ testNumber: details.testNumber });

      li = reporterDocument.createElement( "li" );
      li.appendChild( b );
      li.appendChild( a );
      li.className = "running";
      li.id = "qunit-test-output" + details.testNumber;

      assertList = reporterDocument.createElement( "ol" );
      assertList.className = "qunit-assert-list";
  ...
  QUnit.log(function( details ) {
    ...
    assertLi = reporterDocument.createElement( "li" );
    assertLi.className = details.result ? "pass" : "fail";
    assertLi.innerHTML = message;
    assertList.appendChild( assertLi );
  });
  ...
  QUnit.testDone(function( details ) {
    ...
    time = reporterDocument.createElement( "span" );
    time.className = "runtime";
    time.innerHTML = details.runtime + " ms";
  ...
  if ( !defined.document || reporterDocument.readyState === "complete" ) {
    config.autorun = true;
  }
